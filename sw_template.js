...
self.addEventListener('install', (event) => {
    console.log('[SPSW] Service Worker installed.');
    event.waitUntil(self.skipWaiting()); // 强制新的SW立即激活
});

self.addEventListener('activate', (event) => {
    console.log('[SPSW] Service Worker activated.');
    // 清理旧缓存（如果需要）
...
self.addEventListener('fetch', (event) => {
    ...
    console.log('[SPSW] Intercepting fetch for:', request.url);

    // 优先从我们的自定义映射中查找
    // URL_MAP 的键是原始的绝对URL，值是ZIM内部的相对路径
    if (URL_MAP[request.url]) {
        const localPath = URL_MAP[request.url];
        console.log(`[SPSW] Remapping ${request.url} to local ZIM path: ${localPath}`);
    ...
    console.log(`[SPSW] No exact map match. Guessing ZIM path: ${zimPathGuess}`);
    event.respondWith(
        fetch(zimPathGuess).catch(err => {
            console.error(`[SPSW] Fetch failed for both map and guess (${request.url}):`, err);
            // 如果所有尝试都失败，返回一个网络错误
